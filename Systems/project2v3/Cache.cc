...
Cache::Cache(Memory* passPointer, int passCache, int passBlockSize, int passBlocks)
   {
    pointer = passPointer; // pointer to Memory object
    ...
    for (int i = 0; i < numSets; i++) {
        ...
        sets[i] = new Set(blocksPerSet, blockSize, pointer, decoder, counter); // Allocate each Set
    }
...
Cache::~Cache() {
    ...
    delete counter;// Delete the PerformanceCounter object
}
...
unsigned char Cache::read(unsigned long address) {
    ...
    return sets[setIndex]->read(tag, blockOffset, pointer, address); // Read the value from the Set
}
